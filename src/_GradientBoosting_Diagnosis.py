...
def plot_feature_importance(model, feature_names, top_n=10):
    ...
    file_path = fig_dir / 'gradient_boosting_feature_importances.pdf'
...
def main():
    ...
    # 2. Khởi tạo và Huấn luyện mô hình Gradient Boosting
    print("\nĐang huấn luyện mô hình Gradient Boosting...")

    # Khởi tạo GradientBoostingClassifier (có thể tinh chỉnh siêu tham số ở đây)
    gb_model = GradientBoostingClassifier(
        n_estimators=100,  # Số lượng cây (boosting stages)
        learning_rate=0.1,  # Tốc độ học
        max_depth=3,  # Độ sâu tối đa của từng cây
        random_state=42
    )
    ...
    evaluate_model(gb_model, X_val, y_val, 'Validation')
    ...
    model_filename = model_dir / 'gradient_boosting_model.pkl'
