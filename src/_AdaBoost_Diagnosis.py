...
def plot_feature_importance(model, feature_names, top_n=10):
    ...
    plt.title(f'Top {top_n} Feature Importances - AdaBoost')
    ...
    # Lưu file trước khi show
    file_path = fig_dir / 'adaboost_feature_importances.pdf'
...
def main():
    ...
    # 2. Khởi tạo và Huấn luyện mô hình AdaBoost
    print("\nĐang huấn luyện mô hình AdaBoost...")

    # Khởi tạo AdaBoostClassifier (bạn có thể tinh chỉnh các tham số này)
    adaboost_model = AdaBoostClassifier(
        n_estimators=100,  # Số lượng estimators (mô hình yếu)
    ...
    # 3. Đánh giá mô hình
    evaluate_model(adaboost_model, X_train, y_train, 'Train')
    evaluate_model(adaboost_model, X_val, y_val, 'Validation')
    evaluate_model(adaboost_model, X_test, y_test, 'Test')

    # 4. Trực quan hóa Feature Importance
    plot_feature_importance(adaboost_model, X_train.columns)

    # 5. Lưu mô hình
    model_dir = Path('../models')
    model_dir.mkdir(parents=True, exist_ok=True)

    model_filename = model_dir / 'adaboost_model.pkl'
